...
import { Box, Typography, Paper, Divider, Button, Chip, Tooltip } from "@mui/material";
...
const StudentDashboard = () => {
  ...
  const [gradeMessage, setGradeMessage] = useState("Analyzing your recent performance...");
  ...
  const [aiOverview, setAiOverview] = useState({
    ...
    concern: "Analyzing your weak areas...",
    prediction_reason: "Analyzing your recent performance...",
  });
  ...
  const [studyPattern, setStudyPattern] = useState({
    persona: "Analyzing...",
    proof: "Gathering your study data...",
    suggestion: "Complete more sessions for AI insights.",
    total_time: "0h 0m",
  });
  ...
  const fetchPredictedGrade = async () => {
    try {
      const response = await apiFetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/predict-grade/${user.id}?lang=${language}`
      );
  ...
  const fetchDashboardData = async () => {
    try {
      const response = await apiFetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/student/dashboard/${user.id}?lang=${language}&year=${selectedHeatmapYear}`
      );
      const data = await response.json();
      console.log("Dashboard performance trend:", data.performance_trend);
      ...
      if (data.success) {
        ...
        if (data.topic_recommendations) setRecommendedTopics(data.topic_recommendations);
        if (data.topic_mastery) setTopicMastery(data.topic_mastery);
        if (data.performance_trend) setPerformanceTrend(data.performance_trend);
        if (data.study_schedule) setStudySchedule(data.study_schedule);
        if (data.study_pattern) setStudyPattern(data.study_pattern);
        if (data.consistency) setConsistency(data.consistency);
        if (data.study_heatmap) setStudyHeatmap(data.study_heatmap);
      }
    } catch (error) {
  ...
  useEffect(() => {
    logUserActivity("viewed_dashboard");

    if (user?.id) {
      handleRefreshDashboard();
    }
  }, [user?.id, language, selectedHeatmapYear]);
    ...
    <Box>
      ...
      >
        <Box>
          ...
          >
            {t("dashboard_hello")}, {user?.name || "Student"}
          </Typography>
      ...
      >
        ...
        >
          ...
          >
            {t("dashboard_study_insights_schedule")}
          </Typography>
        </Box>
          ...
          >
            ...
            >
              <Box sx={{ minWidth: 0 }}>
                ...
                >
                  {t("dashboard_consistency_subtitle")}
                </Typography>
              </Box>
        ...
        >
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.8 }}>
              ...
              >
                {t("dashboard_personalized_study_schedule")}
              </Typography>
            </Box>
          ...
          >
            ...
            >
              ...
              >
                {t("dashboard_practice_this_session")}
              </Button>
            </Box>
          </Box>
        </Box>
